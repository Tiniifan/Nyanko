...
{
    // Source code from https://www.codeproject.com/Articles/8086/Extending-the-save-file-dialog-class-in-NET
    ...
    public class SaveFileDialogWithEncoding
    {
        ...
        private int m_VarianceCheckHandle = 0;
        private int m_SaveKeysCheckHandle = 0;
        ...
        private bool m_VarianceKeySupport = false;
        private bool m_SaveKeysList = true;
        ...
        private const int BS_AUTOCHECKBOX = 0x0003;
        ...
        private int HookProc(int hdlg, int msg, int wParam, int lParam)
        {
            switch (msg)
            {
                case WM_INITDIALOG:
                    ...
                    // Extra height now accounts for the encoding row PLUS the two new checkbox rows
                    SetWindowPos(parent, 0, x, y, cr.Right - cr.Left, cr.Bottom - cr.Top + 80, SWP_NOZORDER);
                    ...
                    SendMessage(comboHandle, CB_ADDSTRING, 0, "Shift-JIS");

                    SendMessage(comboHandle, CB_SETCURSEL, (int)m_EncodingType, 0);

                    // Create the "Support variance keys" checkbox right below the encoding combo
                    int varianceCheckHandle = CreateWindowEx(0, "BUTTON", "Support variance keys",
                        WS_VISIBLE | WS_CHILD | WS_TABSTOP | (uint)BS_AUTOCHECKBOX,
                        point.X, point.Y + 32, rightPoint.X - point.X, 20, parent, 0, 0, 0);
                    SendMessage(varianceCheckHandle, WM_SETFONT, fontHandle, 0);
                    SendMessage(varianceCheckHandle, BM_SETCHECK, m_VarianceKeySupport ? BST_CHECKED : BST_UNCHECKED, 0);

                    // Create the "Save keys list" checkbox right below the variance checkbox
                    int saveKeysCheckHandle = CreateWindowEx(0, "BUTTON", "Save keys list",
                        WS_VISIBLE | WS_CHILD | WS_TABSTOP | (uint)BS_AUTOCHECKBOX,
                        point.X, point.Y + 56, rightPoint.X - point.X, 20, parent, 0, 0, 0);
                    SendMessage(saveKeysCheckHandle, WM_SETFONT, fontHandle, 0);
                    SendMessage(saveKeysCheckHandle, BM_SETCHECK, m_SaveKeysList ? BST_CHECKED : BST_UNCHECKED, 0);
                    ...
                    m_VarianceCheckHandle = varianceCheckHandle;
                    m_SaveKeysCheckHandle = saveKeysCheckHandle;
                ...
                case WM_DESTROY:
                    ...
                    if (m_VarianceCheckHandle != 0)
                    {
                        DestroyWindow(m_VarianceCheckHandle);
                    }

                    if (m_SaveKeysCheckHandle != 0)
                    {
                        DestroyWindow(m_SaveKeysCheckHandle);
                    }
                    break;
                case WM_NOTIFY:
                    ...
                    if (nmhdr.Code == CDN_FILEOK)
                    {
                        ...
                        // Read back the state of the two checkboxes
                        m_VarianceKeySupport = SendMessage(m_VarianceCheckHandle, BM_GETCHECK, 0, 0) == BST_CHECKED;
                        m_SaveKeysList = SendMessage(m_SaveKeysCheckHandle, BM_GETCHECK, 0, 0) == BST_CHECKED;
                    }
                    break;
        ...
        public bool VarianceKeySupport
        {
            get
            {
                return m_VarianceKeySupport;
            }
            set
            {
                m_VarianceKeySupport = value;
            }
        }

        public bool SaveKeysList
        {
            get
            {
                return m_SaveKeysList;
            }
            set
            {
                m_SaveKeysList = value;
            }
        }
        ...
        public DialogResult ShowDialog()
        {
            ...
            ofn.lpstrInitialDir = m_InitialDirectory;
            ofn.nFilterIndex = m_FilterIndex;  // Set the filter index
            ...
            m_InitialDirectory = ofn.lpstrInitialDir;
